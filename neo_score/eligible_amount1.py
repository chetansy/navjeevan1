# -*- coding: utf-8 -*-
"""
Created on Thu Sep 21 13:59:28 2023
...
file_path = r"training_data_eligible.csv"
df = pd.read_csv(file_path)

df.drop(columns=['required_credit_amount'], inplace=True)
...
scaler = MinMaxScaler()
df[df.columns.difference(['eligible_amount'])] = scaler.fit_transform(df[df.columns.difference(['eligible_amount'])])
...
X = df.drop(columns=['eligible_amount'])
y = df['eligible_amount']
print(X)
print(y)
print(df.info())
...
print(f"Mean Squared Error: {mse}")
 ...
 # Saving
with open('eligible_amount_model_and_transformers1.pkl', 'wb') as f:
